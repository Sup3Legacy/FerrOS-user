...
pub fn build_tree(vector: Vec<String>) -> Result<command::Command, ()> {
    ...
    for lexem in vector.iter() {
        if lexem == "&&" {
            ...
            out = Some(match out {
                None => command2,
                Some(command) => command::Command::Connection(
                    Box::new(command),
    ...
    match out {
        ...
        Some(command) => match build_tree_and(line) {
            Err(()) => Err(()),
            Ok(command2) => Ok(command::Command::Connection(
                Box::new(command),
                command::Connector::Seq,
                Box::new(command2),
            )),
...
fn build_tree_and(vector: Vec<String>) -> Result<command::Command, ()> {
    ...
    match out {
        ...
        Some(command) => match build_tree_or(line) {
            Err(()) => Err(()),
            Ok(command2) => Ok(command::Command::Connection(
                Box::new(command),
                command::Connector::And,
...
fn build_tree_or(vector: Vec<String>) -> Result<command::Command, ()> {
    ...
    match out {
        ...
        Some(command) => match build_tree_pipe(line) {
            Err(()) => Err(()),
            Ok(command2) => Ok(command::Command::Connection(
                Box::new(command),
                command::Connector::Or,
...
fn build_tree_pipe(vector: Vec<String>) -> Result<command::Command, ()> {
    ...
    match out {
        ...
        Some(command) => match build_tree_low(line) {
            Err(()) => Err(()),
            Ok(command2) => Ok(command::Command::Connection(
                Box::new(command),
                command::Connector::Pipe,
