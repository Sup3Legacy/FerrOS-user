...
fn reduce(num:usize, mut list_etat: Vec<usize>, mut pile: Vec<RulesType>) -> ReturnReduce {
	match num {
		21 => {
			let l2 = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let l1 = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::LIST1({ command::Command::Connection (Box::new(l1), command::Connector::And, Box::new(l2)) }));
			ReturnReduce::Success(list_etat, pile, "list1")
		}
		34 => {
			let sce = match pile.pop() { Some(RulesType::SIMPLE_COMMAND_ELEMENT(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			let sc = match pile.pop() { Some(RulesType::SIMPLE_COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_COMMAND({ make_simple_command(sce, sc) }));
			ReturnReduce::Success(list_etat, pile, "simple_command")
		}
		13 => {
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let cl3 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let cl2 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let cl1 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..7 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::IF_COMMAND({ command::Command::If (Box::new(cl1), Box::new(cl2), Box::new(cl3)) }));
    ...
    command::Command::Connection (Box::new(l1.make_async()), command::Connector::Seq, Box::new(l2))
		...
		33 => {
			let ifc = match pile.pop() { Some(RulesType::IF_COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SHELL_COMMAND({ ifc }));
			ReturnReduce::Success(list_etat, pile, "shell_command")
		}
		42 => {
			let sl2 = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let sl1 = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_LIST1({ command::Command::Connection (Box::new(sl1), command::Connector::Or, Box::new(sl2)) }));
		...
		26 => {
			...
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::NEWLINE_LIST({ MyUnit::MyUnit }));
			ReturnReduce::Success(list_etat, pile, "newline_list")
		}
		15 => {
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let ec = match pile.pop() { Some(RulesType::ELIF_CLAUSE(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			let cl2 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let cl1 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..6 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::IF_COMMAND({ command::Command::If (Box::new(cl1), Box::new(cl2), Box::new(ec)) }));
			ReturnReduce::Success(list_etat, pile, "if_command")
		}
		45 => {
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_LIST_TERMINATOR({ MyUnit::MyUnit }));
			ReturnReduce::Success(list_etat, pile, "simple_list_terminator")
		}
		36 => {
			let w = match pile.pop() { Some(RulesType::Tok(Token::WORD(t))) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_COMMAND_ELEMENT({ SimpleCommandElement::EltWord(w) }));
		...
		23 => {
			...
			let l1 = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::LIST1({ command::Command::Connection (Box::new(l1), command::Connector::Or, Box::new(l2)) }));
			ReturnReduce::Success(list_etat, pile, "list1")
		}
		43 => {
			let sl2 = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let sl1 = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..3 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_LIST1({ command::Command::Connection (Box::new(sl1), command::Connector::Seq, Box::new(sl2)) }));
			ReturnReduce::Success(list_etat, pile, "simple_list1")
		}
		0 => {
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let l = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..3 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		4 => {
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let sl = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_LIST({ sl }));
			ReturnReduce::Success(list_etat, pile, "simple_list")
		}
		28 => {
			let p2 = match pile.pop() { Some(RulesType::PIPELINE(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let p1 = match pile.pop() { Some(RulesType::PIPELINE(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::PIPELINE({ command::Command::Connection(Box::new(p1), command::Connector::Pipe, Box::new(p2)) }));
		...
		31 => {
			...
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::REDIRECTION({ command::Redirect::OutputAppend(w) }));
		...
		24 => {
			...
			let l1 = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::LIST1({ command::Command::Connection (Box::new(l1), command::Connector::Seq, Box::new(l2)) }));
			ReturnReduce::Success(list_etat, pile, "list1")
		}
		8 => {
			let l = match pile.pop() { Some(RulesType::LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		18 => {
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			let sl = match pile.pop() { Some(RulesType::SIMPLE_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
      ...
      command::Command::Connection (Box::new(sl1.make_async()), command::Connector::Seq, Box::new(sl2))
		...
		19 => {
			let l = match pile.pop() { Some(RulesType::LIST0(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..2 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		7 => {
			for _i in 0..0 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::NEWLINE_LIST({ MyUnit::MyUnit }));
			ReturnReduce::Success(list_etat, pile, "newline_list")
		}
		27 => {
			let c = match pile.pop() { Some(RulesType::COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		10 => {
			...
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::ELIF_CLAUSE({ command::Command::If(Box::new(cl1), Box::new(cl2), Box::new(command::Command::Nothing)) }));
		...
		12 => {
			...
			let cl1 = match pile.pop() { Some(RulesType::COMPOUND_LIST(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..5 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::ELIF_CLAUSE({ command::Command::If(Box::new(cl1), Box::new(cl2), Box::new(ec)) }));
			ReturnReduce::Success(list_etat, pile, "elif_clause")
		}
		32 => {
			let w = match pile.pop() { Some(RulesType::Tok(Token::WORD(t))) => t, _ => return ReturnReduce::SamenhirErrorReduce};
		...
		37 => {
			let r = match pile.pop() { Some(RulesType::REDIRECTION(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_COMMAND_ELEMENT({ SimpleCommandElement::EltRedi(r) }));
			ReturnReduce::Success(list_etat, pile, "simple_command_element")
		}
		35 => {
			let sce = match pile.pop() { Some(RulesType::SIMPLE_COMMAND_ELEMENT(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_COMMAND({ match sce {
        SimpleCommandElement::EltWord(w) =>
		...
		14 => {
			...
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..5 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::IF_COMMAND({ command::Command::If(Box::new(cl1), Box::new(cl2), Box::new(command::Command::Nothing)) }));
			ReturnReduce::Success(list_etat, pile, "if_command")
		}
		22 => {
			let l2 = match pile.pop() { Some(RulesType::LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
		...
		11 => {
			...
			match pile.pop() { Some(RulesType::Tok(_)) => (), _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..6 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::ELIF_CLAUSE({ command::Command::If (Box::new(cl1), Box::new(cl2), Box::new(cl3)) }));
			ReturnReduce::Success(list_etat, pile, "elif_clause")
		}
		1 => {
			match pile.pop() { Some(RulesType::Tok(_)) => return ReturnReduce::SamenhirErrorReduce, Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce};
		...
		25 => {
			let pc = match pile.pop() { Some(RulesType::PIPELINE_COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		41 => {
			...
			let sl1 = match pile.pop() { Some(RulesType::SIMPLE_LIST1(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..4 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::SIMPLE_LIST1({ command::Command::Connection (Box::new(sl1), command::Connector::And, Box::new(sl2)) }));
			ReturnReduce::Success(list_etat, pile, "simple_list1")
		}
		6 => {
			let sc = match pile.pop() { Some(RulesType::SIMPLE_COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::COMMAND({ command::Command::SimpleCommand(clean_simple_command(sc)) }));
		...
		29 => {
			let p = match pile.pop() { Some(RulesType::PIPELINE(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
		...
		5 => {
			let shc = match pile.pop() { Some(RulesType::SHELL_COMMAND(t)) => t, _ => return ReturnReduce::SamenhirErrorReduce};
			for _i in 0..1 {match list_etat.pop() { Some(_) => (), _ => return ReturnReduce::SamenhirErrorReduce}};
			pile.push(RulesType::COMMAND({ unsafe { FIRST_WORD = true}; shc }));
			ReturnReduce::Success(list_etat, pile, "command")
		}
...
fn action_all(new_token : fn (&mut super::lexer::Lexbuf) -> Result<Token, &'static str>, lexbuf : &mut super::lexer::Lexbuf, mut etat : usize) -> Result<command::Command, Errors> {
...
pub fn inputunit(lexer : fn (&mut super::lexer::Lexbuf) -> Result<Token, &'static str>, lexbuf : &mut super::lexer::Lexbuf) -> Result<command::Command, Errors> {
