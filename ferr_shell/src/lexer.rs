...
impl Lexbuf {
    ...
    fn reset_current(&mut self) {
        self.current = self.pos_e_cnum;
    }

    fn get_read(&mut self) -> usize {
        self.read
    }
...
fn token(lexbuf: &mut Lexbuf) -> Result<parser::Token, &'static str> {
    if lexbuf.finished() {
        Ok(parser::Token::EOF)
    } else {
        lexbuf.new_extremity();
        match token_17(lexbuf) {
            _ => {
                let i = lexbuf.get_read();
                lexbuf.reset_current();
                lexbuf.set_read(0);
                if lexbuf.pos_e_cnum <= lexbuf.pos_b_cnum {
                    Err("Reached End of File without any corrresponding rule")
                } else if i == 15 {
                    comment(lexbuf);
                    token(lexbuf)
                } else if i == 17 {
                    lexbuf.newline();
                    Ok(parser::Token::NEWLINE)
                } else if i == 20 {
                    token(lexbuf)
                } else if i == 22 {
                    ...
                    let s = lexbuf.get_token();
                    match (unsafe { parser::FIRST_WORD }, keyword_of_string(&s)) {
                        (true, Some(kw)) => Ok(kw),
                        _ => {
                    ...
                    Err("Undefined rule, should not happen, please report this")
                }
            }
        }
    }
...
fn token_1(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    ...
    if let Ok(chr) = lexbuf.next_char() {
            ...
            || (9 <= chr && chr <= 10)
        {
            ...
            token_14(lexbuf.incr_pos())
        } else if (125 <= chr && chr <= 255)
            || (93 <= chr && chr <= 123)
            || (63 <= chr && chr <= 91)
            || chr == 61
            || (39 <= chr && chr <= 58)
...
fn token_5(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(24);
    if let Ok(chr) = lexbuf.next_char() {
        if (63 <= chr && chr <= 255) || (0 <= chr && chr <= 61) {
            token_0(lexbuf.incr_pos())
        } else if chr == 62 {
            token_6(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn token_7(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(29);
    if let Ok(chr) = lexbuf.next_char() {
        if (39 <= chr && chr <= 255) || (0 <= chr && chr <= 37) {
            token_0(lexbuf.incr_pos())
        } else if chr == 38 {
            token_8(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn token_10(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(37);
    if let Ok(chr) = lexbuf.next_char() {
        if (125 <= chr && chr <= 255) || (0 <= chr && chr <= 123) {
            token_0(lexbuf.incr_pos())
        } else if chr == 124 {
            token_9(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn token_16(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(48);
    if let Ok(chr) = lexbuf.next_char() {
        if (0 <= chr && chr <= 255) {
            token_0(lexbuf.incr_pos())
...
fn token_17(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if chr == 35 {
            token_1(lexbuf.incr_pos())
        } else if chr == 10 {
            token_2(lexbuf.incr_pos())
        } else if chr == 32 || chr == 9 {
            token_3(lexbuf.incr_pos())
        } else if chr == 60 {
            token_4(lexbuf.incr_pos())
        } else if chr == 62 {
            token_5(lexbuf.incr_pos())
        } else if chr == 38 {
            token_7(lexbuf.incr_pos())
        } else if chr == 124 {
            token_10(lexbuf.incr_pos())
        } else if chr == 59 {
            token_11(lexbuf.incr_pos())
        } else if chr == 92 {
            token_12(lexbuf.incr_pos())
        } else if (125 <= chr && chr <= 255)
            || (93 <= chr && chr <= 123)
            || (63 <= chr && chr <= 91)
            || chr == 61
            ...
            || (0 <= chr && chr <= 8)
        {
            token_13(lexbuf.incr_pos())
        } else if chr == 34 {
            token_16(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn word(lexbuf: &mut Lexbuf) -> Result<parser::Token, &'static str> {
    if lexbuf.finished() {
...
fn word_5(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(59);
    if let Ok(chr) = lexbuf.next_char() {
        if (0 <= chr && chr <= 255) {
            word_3(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn word_6(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if chr == 124
            || chr == 62
            || (59 <= chr && chr <= 60)
            ...
            || (9 <= chr && chr <= 10)
        {
            word_1(lexbuf.incr_pos())
        } else if chr == 34 {
            word_2(lexbuf.incr_pos())
        } else if (125 <= chr && chr <= 255)
            || (93 <= chr && chr <= 123)
            || (63 <= chr && chr <= 91)
            || chr == 61
            ...
            || (0 <= chr && chr <= 8)
        {
            word_4(lexbuf.incr_pos())
        } else if chr == 92 {
            word_5(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn double_quote(lexbuf: &mut Lexbuf) -> Result<parser::Token, &'static str> {
    if lexbuf.finished() {
        panic!("Unclosed String")
    } else {
        lexbuf.new_extremity();
        match double_quote_5(lexbuf) {
            _ => {
                let i = lexbuf.get_read();
                lexbuf.reset_current();
                lexbuf.set_read(0);
                if lexbuf.pos_e_cnum <= lexbuf.pos_b_cnum {
                    ...
                    let t1 = lexbuf.get_token();
                    Ok(parser::Token::WORD(String::new()))
                } else if i == 65 {
                    let t2 = lexbuf.get_token();
                    let mut s1 = String::from("\"");
                    match double_quote(lexbuf) {
                        Ok(parser::Token::WORD(s)) => s1.push_str(&s),
                        _ => panic!("should not happen"),
...
fn double_quote_0(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if (0 <= chr && chr <= 255) {
            double_quote_0(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn double_quote_4(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(71);
    if let Ok(chr) = lexbuf.next_char() {
        if chr == 92 || chr == 34 || chr == 10 {
            double_quote_0(lexbuf.incr_pos())
        } else if (93 <= chr && chr <= 255)
            || (35 <= chr && chr <= 91)
            || (11 <= chr && chr <= 33)
            || (0 <= chr && chr <= 9)
        {
            double_quote_4(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn double_quote_5(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if chr == 10 {
            double_quote_1(lexbuf.incr_pos())
        } else if chr == 34 {
            double_quote_2(lexbuf.incr_pos())
        } else if (93 <= chr && chr <= 255)
            || (35 <= chr && chr <= 91)
            || (11 <= chr && chr <= 33)
            || (0 <= chr && chr <= 9)
        {
            ...
            double_quote_6(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn double_quote_6(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if (0 <= chr && chr <= 255) {
            double_quote_3(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
...
fn comment(lexbuf: &mut Lexbuf) -> Result<parser::Token, &'static str> {
    if lexbuf.finished() {
...
fn comment_2(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    lexbuf.set_read(75);
    if let Ok(chr) = lexbuf.next_char() {
        if (0 <= chr && chr <= 255) {
            comment_0(lexbuf.incr_pos())
...
fn comment_3(lexbuf: &mut Lexbuf) -> Result<(), ()> {
    if let Ok(chr) = lexbuf.next_char() {
        if chr == 10 {
            comment_1(lexbuf.incr_pos())
        } else if (11 <= chr && chr <= 255) || (0 <= chr && chr <= 9) {
            comment_2(lexbuf.incr_pos())
        } else {
            Err(())
        }
    } else {
